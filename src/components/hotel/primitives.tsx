...
export function Panel({
  ...
  return (
    <section
      id={id}
      className={cn(
        "flex scroll-mt-40 flex-col overflow-hidden rounded-3xl border border-border/70 bg-surface shadow-[var(--shadow-card)] transition-shadow duration-300 hover:shadow-[var(--shadow-float)]",
        className,
      )}
    >
      {title || action ? (
      ...
      {footer ? (
        <div className="border-t border-border bg-surface-muted px-4 py-2.5">{footer}</div>
      ) : null}
    </section>
  );
}
